...
module.exports = function (app) {
	...
	// format /api/account
    app.route('/api/account').get([accountPolicy.isAllowed, ipLogger.log], accountController.read);

    // GET gets user's edit profile information
	// format /api/edit-profile
    app.route('/api/edit-profile').get([accountPolicy.isAllowed, ipLogger.log], accountController.read);

    // GET gets user's change password information
	// format /api/change-password
    app.route('/api/change-password').get([accountPolicy.isAllowed, ipLogger.log], accountController.read);

    // GET gets user's hub information
	// format /api/hub
    app.route('/api/hub').get([accountPolicy.isAllowed, ipLogger.log], accountController.read);

    // GET gets user's notifications information
	// format /api/notifications
    app.route('/api/notifications').get([accountPolicy.isAllowed, ipLogger.log], accountController.read);
};